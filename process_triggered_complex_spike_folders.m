...
for ii = 1:length(folders)
    ...
    for kk = 1 :size(cs_waves,1)
        idx = argfindpeaks(cs_waves(kk,:),CSTHRESH,int32(0.5./diff(twave(1:2))));
    ...
    for kk = 1:length(unique_n)
        ...
        plot(twave,cs_waves(n_spikelets==unique_n(kk),:),'color',cc(kk,:),'linewidth',0.6)
    end
    ...
    appendix(appendix =='_') = '-';
    caption = sprintf(['Experiment %s. Mean intercomplex spike interval: ',...
        '%3.2f s. A - Interspikelet interval versus the timing of the ',...
        'spikelet from the complex spike onset. Color codes for the max',...' number of spikelets in the CS waveform. B - Histogram of the ',...
...
if exist('spikelet_number','var')
    ...
    bins = histc(iCSi(spikelet_count==spikelet_number),edges);
    bar(edges,bins,'facecolor','k')
    ...
    try
        ...
        disp('Cant set ylim... intervals empty?')
        keyboard
    end
    %     MAX_SPIKELET = 6;
    %    MAX_SPIKELET = 50;
    ...
    for ii = 1:length(intervals)
        idx = (iCSi>(intervals(ii)) &...
            iCSi<=(intervals(ii)+iCSi_BIN)) &...
             (spikelet_count'==spikelet_number) &...
             (tmp<MAX_SPIKELET);
        axes(ax(1))
        plot(twave,cs_waves(idx,:)+offset*(ii-1),'color',cc(ii,:))
        axes(ax(2))
        
        plot(cellfun(@(x)x(spikelet_number),spikelets_time(idx)),cellfun(@(x)x(spikelet_number),spikelets_isi(idx)),...
            'ko','markeredgecolor',cc(ii,:),'markerfacecolor','none')
        axes(ax(4))
        plot(iCSi(idx),cellfun(@(x)x(spikelet_number),spikelets_isi(idx)),...
            'ko','markeredgecolor',cc(ii,:),'markerfacecolor','none')
        mean_iCSi(ii) = mean(iCSi(idx));
        mean_spikelet_isi(ii) = mean(cellfun(@(x)x(spikelet_number),spikelets_isi(idx)));
        std_spikelet_isi(ii) = std(cellfun(@(x)x(spikelet_number),spikelets_isi(idx)));%/sqrt(length(idx));
        
        fid.cs_waves = [fid.cs_waves;cs_waves(idx,:)];
        fid.interCSinterval = [fid.interCSinterval;iCSi(idx)];
    end
    axes(ax(4))
    errorbar(mean_iCSi,mean_spikelet_isi,std_spikelet_isi,'k','markerfacecolor',[.5,.5,.5])
    axis tight
    axes(ax(1))
    axis tight
    xlim([min(xlim),15])
    ...
    caption = sprintf(['Experiment %s. A - Interspikelet interval versus the timing of the ',...
        ...
        expName,spikelet_number,spikelet_number,fname);
